...
def create_output_directory(hydrofab_path):
    ...
    match = re.search(r'(.+)\.gpkg$', filename)
    if match:
        base_name = match.group(1)  # Everything before .gpkg
        # clean up the filename
        clean_name = re.sub(r'^(gages-|gauge_)', '', base_name)
        output_dir = f"/ngen-app/data/AORC_2.2_test/Gage_{clean_name}"
...
def execute(args):
    ...
    NextGen_lumped_forcings_driver(
        ...
        end_time="2023-01-01 00:00:00", # one timstep past target end-time
        met_dataset="AORC",
        hyfabfile=hyfab_in,
