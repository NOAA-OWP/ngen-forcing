...
def get_options():
    ...
    parser.add_argument('csv_US', type=pathlib.Path)
    parser.add_argument('csv_Can', type=pathlib.Path)
...
def main_US(args):
    ...
    with open(f"{args.out_prefix}source_sink_BMI_US.in", 'w') as out:
        srcs = pts.FlowDir == 1
        sinks = pts.FlowDir == -1
...
def main_Can(args):
    ...
    with open(f"{args.out_prefix}source_sink_BMI_Can.in", 'w') as out:
        srcs = pts.FlowDir == 1
        sinks = pts.FlowDir == -1
