...
FVCOM_dir = "/scratch2/NCEPDEV/ohd/Jason.Ducker/Lake_Ontario/FVCOM/Lake_Ontario/"
gridFile = "/scratch2/NCEPDEV/ohd/NG_Coastal_Model_Setups/Lake_Ontario/SCHISM/hgrid.gr3"
outFile = '/scratch2/NCEPDEV/ohd/NG_Coastal_Model_Setups/Lake_Ontario/SCHISM/elev2Dth.nc'
#convFile = path+'Data/domainFiles/atl/final/atlBoundNodes_conversion_factor.txt'
start_time = datetime.strptime("2019-04-01 00:00", '%Y-%m-%d %H:%M')
end_time = datetime.strptime("2019-10-15 00:00", '%Y-%m-%d %H:%M')
########################################################################################
...
FVCOM_outfile = "glofs.loofs.fields.nowcast."
...
units = [('time', ref_time),
         ('waterlevelbnd', 'm')]
zeta = np.empty((len(FVCOM_daterange)*6,len(schism_bnds[:,0])),dtype=float)
...
for i in range(len(FVCOM_daterange)):
    ...
    _, stations = kd_nearest_neighbor(adpts, schism_bnds)
    print(ds.variables['zeta'][:].max())
