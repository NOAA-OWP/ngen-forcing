...
CERA = "/scratch2/NCEPDEV/ohd/Jason.Ducker/Atlantic_Data/Florence_fort.63.nc"
gridFile = "/scratch2/NCEPDEV/ohd/Camaron.George/Data/domainFiles/atl/hgrid.gr3"
outFile = '/scratch2/NCEPDEV/ohd/NG_Coastal_Model_Setups/Atlantic+Gulf/SCHISM/elev2Dth.nc'
#convFile = path+'Data/domainFiles/atl/final/atlBoundNodes_conversion_factor.txt'
start_time = datetime.strptime("2018-09-12 00:00", '%Y-%m-%d %H:%M')
end_time = datetime.strptime("2018-09-22 00:00", '%Y-%m-%d %H:%M')
########################################################################################
...
ds = nc.Dataset(CERA, mode='r')
CERA_zeta = ds.variables['zeta'][:]
print("Masking invalid stations")
#mask = invalid_mask(CERA_zeta)
#valid_stations = mask.nonzero()[0]
adcirc = pd.DataFrame([])
adcirc['adlons'] = ds.variables['x'][:]
adcirc['adlats'] = ds.variables['y'][:]
adcirc['mask_lon'] = False
adcirc['mask_lat'] = False
idx = (adcirc.adlons <= np.max(lon)+0.01) & (adcirc.adlons >= np.min(lon)-0.01)
adcirc.loc[idx,'mask_lon'] = True
idx = (adcirc.adlats <= np.max(lat)+0.01) & (adcirc.adlats >= np.min(lat)-0.01)
adcirc.loc[idx,'mask_lat'] = True

idx = (adcirc.mask_lon == True) & (adcirc.mask_lat == True)
adlons = adcirc.loc[idx,'adlons'].values
adlats = adcirc.loc[idx,'adlats'].values
...
CERA_zeta = CERA_zeta[:,idx.values]
CERA_zeta = CERA_zeta[:,CN]
time_col = ds.variables['time']
ref_time = time_col.units.rstrip('UTC').rstrip()


time_final = nc.num2date(time_col[:],units=ref_time,only_use_cftime_datetimes=False)


zeta = np.empty((len(CERA_daterange),len(lon)),dtype=float)

for i in range(len(CERA_daterange)):
    if(CERA_daterange[i] in time_final):
        idx_time = np.where(time_final==CERA_daterange[i])[0][0]
        zeta[i,:] = CERA_zeta[idx_time,:]
    else:
        zeta[i,:] = tidal_wl(CERA_daterange[i],amplitude,phase)
