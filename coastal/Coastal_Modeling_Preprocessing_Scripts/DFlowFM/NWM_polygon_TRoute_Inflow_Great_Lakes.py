...
def import_streamline_csv(hyfab_csv_file):
    ...
    #hyfab['POIS'] = extract_pois(hyfab.hl_link.values)#np.array([i.split('-')[-1] for i in hyfab.hl_link.values],dtype='float')
    hyfab['POIS'] = np.array([i.split('-')[-1] for i in hyfab.flowpath_id.values],dtype='int')
    idx = hyfab.outside == 0
    hyfab = hyfab.loc[idx,:]
    #idx = hyfab.flag != "outside domain"
    #hyfab = hyfab.loc[idx,:]
    return hyfab
...
def get_options():
    ...
    parser.add_argument('--comm_ids_US', dest='comm_id_path_US', required=True,
                    help='The file list the US-side comm ids to be extracted. Two columns map the comm id to an identifier string (used for boundary condition output).')
    parser.add_argument('--comm_ids_Can', dest='comm_id_path_Can', required=True,
                    help='The file list the Canadian-side comm ids to be extracted. Two columns map the comm id to an identifier string (used for boundary condition output).')
    parser.add_argument('--NWM_polygon_US', dest='NWM_polygon_US', required=True, type=pathlib.Path, default=None,
                    help='GIS produced US-side shape file that speifies the NWM polygon coordinates based on the D-Flow extent of its meshes.')
    parser.add_argument('--NWM_polygon_Can', dest='NWM_polygon_Can', required=True, type=pathlib.Path, default=None,
                    help='GIS produced Canadian-side shape file that speifies the NWM polygon coordinates based on the D-Flow extent of its meshes.')
    parser.add_argument('--polygon_US', dest='polygon_US', required=True, type=pathlib.Path,
                    help='The path of the US-side polygon file defining the region of interest.')
    parser.add_argument('--polygon_Can', dest='polygon_Can', required=True, type=pathlib.Path,
                    help='The path of the Canadian-side polygon file defining the region of interest.')
    ...
    parser.add_argument('--troute_input_US', dest='troute_input_dir_US', required=True, type=pathlib.Path,
                    help='The directory that stores US-side of TRoute streamflow input files')
    parser.add_argument('--troute_input_Can', dest='troute_input_dir_Can', required=True, type=pathlib.Path,
                    help='The directory that stores Canadian-side of TRoute streamflow input files')
...
def main(args):
    ...
    domain_polygon_US = read_polygon(args.polygon_US)
    domain_polygon_Can = read_polygon(args.polygon_Can)

    # Read csv file with HUC-12 ID corresponding to NWM Common IDs
    NWM_streams_US = import_streamline_csv(args.comm_ids_US)
    NWM_streams_Can = import_streamline_csv(args.comm_ids_Can)

    # create latq ext file using "new" ext file format
    NWM_polygon_ext(args.ext, NWM_streams_US, args.NWM_polygon_US, args.output)
    NWM_polygon_ext(args.ext, NWM_streams_Can, args.NWM_polygon_Can, args.output)

    ############### BEGIN LATQ NWM file extraction ##################
    #get the path for the first input file
    files_US = get_inputfiles(args.troute_input_US, args.start_time, args.stop_time)
    f0_US = next(iter(files_US.values()))

    files_US = get_inputfiles(args.troute_input_US, args.start_time, args.stop_time)
    f0_US = next(iter(files_US.values()))

    files_Can = get_inputfiles(args.troute_input_Can, args.start_time, args.stop_time)
    f0_Can = next(iter(files_Can.values()))

    print("Processing", len(files_US), "files in", args.troute_input_US)
    ...
    feature_ids_US = np.array(NWM_streams_US['poi_id'].values,dtype=float)
    feature_ids_Can = np.array(NWM_streams_Can['poi_id'].values,dtype=float)
    ...
    troute_source_ids = np.empty((len(feature_ids_US),2))
    troute_source_ids[:,0] = feature_ids_US
    troute_source_ids[:,1] = feature_ids_US

    distance, mask_US = spatial.KDTree(feature_ids_inds).query(troute_source_ids)
    ...
    troute_source_ids = np.empty((len(feature_ids_Can),2))
    troute_source_ids[:,0] = feature_ids_Can
    troute_source_ids[:,1] = feature_ids_Can

    distance, mask_Can = spatial.KDTree(feature_ids_inds).query(troute_source_ids)



    # add nwmcommid as string for

    ids_final = np.array(np.concatenate([NWM_streams_US.POIS.values,NWM_streams_Can.POIS.values]),dtype=str)

    flowdir = np.array(np.concatenate([NWM_streams_US.FlowDir.values,NWM_streams_Can.FlowDir.values]),dtype=float)
    #extract lat long and streamflow for the ids with stored offsets
    inflows_timeseries = np.ma.masked_array(np.zeros((len(ids_final), len(files_US)*12)), fill_value=0.0)

    timestamps_timeseries = []#np.empty((len(files), 12))

    inflows = np.ma.masked_array(np.zeros((len(ids_final), len(time_final))), fill_value=0.0)

    data = {'ids': ids_final,
            'col_index': ids_final,
            'row_index': list(files_US.keys())}
    ...
    troute_index = 0
    for i in range(ffiles):
        f_US = files_US_final[i]
        inflows_timeseries[0:len(feature_ids_US),troute_index:troute_index+12], timestamps_file = extract_inflow_NWM(f_US,mask_US)
        f_Can = files_Can_final[i]
        inflows_timeseries[len(feature_ids_US):,troute_index:troute_index+12], timestamps_file = extract_inflow_NWM(f_Can,mask_Can)
        troute_index += 12
    ...
    for i in range(len(time_final)):
        ...
        inflows[:,i] = inflows_timeseries[:,index]*flowdir
    # Set streamflow data
    data["inflow"] = inflows
