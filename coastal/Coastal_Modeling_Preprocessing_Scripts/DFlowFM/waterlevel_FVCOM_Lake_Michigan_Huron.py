    ...
    def add_forcing(self, name, function, units, Lake, data, vectors=None):
        ...
        if(Lake != None):
            if(Lake.upper() == 'ERIE'):
                ...
                offset = "183.2"
            elif(Lake.upper() == 'MICHIGAN_HURON'):
                offset = "176.0"
...
def main(args):
    ...
    FVCOM_lons =  FVCOM_data.variables['lon'][:].data#np.ma.getdata((FVCOM_data.variables['lon'][:]+180) % 360 - 180)
    FVCOM_lats =  FVCOM_data.variables['lat'][:].data#np.ma.getdata(FVCOM_data.variables['lat'][:])
    ...
    for i in range(len(FVCOM_daterange)):
        for hour in forecast_range:
            ...
            zeta[count,:] = ds.variables['zeta'][:].data[0,:]
