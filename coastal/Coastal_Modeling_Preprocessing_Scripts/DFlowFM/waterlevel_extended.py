...
def main(args):
    ...
    ds1 = nc.Dataset(args.CERA1, mode='r')
    ds2 = nc.Dataset(args.CERA2, mode='r')
    ...
    #mask = np.where(ds1.variables['x'] < np.nanmin(pli_data['values'][:,0])-1, False, mask)
    #mask = np.where(ds1.variables['x'] > np.nanmax(pli_data['values'][:,0])+1, False, mask)
    #mask = np.where(ds1.variables['y'] < np.nanmin(pli_data['values'][:,1])-1, False, mask)
    #mask = np.where(ds1.variables['y'] > np.nanmax(pli_data['values'][:,1])+1, False, mask)
    ...
    print(f"{len(mask) - len(valid_stations)} of {len(mask)} discarded")
    print("Masking coordinates")
    #mask = np.where(ds2.variables['x'] < np.nanmin(pli_data['values'][:,0])-1, False, mask)
    #mask = np.where(ds2.variables['x'] > np.nanmax(pli_data['values'][:,0])+1, False, mask)
    #mask = np.where(ds2.variables['y'] < np.nanmin(pli_data['values'][:,1])-1, False, mask)
    #mask = np.where(ds2.variables['y'] > np.nanmax(pli_data['values'][:,1])+1, False, mask)
...
def get_options():
    ...
    parser.add_argument('CERA1', type=pathlib.Path, help="First CERA file fort.63.nc path")
    parser.add_argument('CERA2', type=pathlib.Path, help="Second CERA file fort.63.nc path")
