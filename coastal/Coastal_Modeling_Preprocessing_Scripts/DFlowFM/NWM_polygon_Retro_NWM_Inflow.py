...
def import_streamline_csv(HUC_csv_file):
    ...
    #HUC_NWM_agg['NWMcommID'] = HUC_NWM_agg['ID_NWMv3']
...
def NWM_polygon_ext(ext_file, HUC_NWM_agg_sliced, NWM_polygon_file,bc_file):
    ...
    for i, row in HUC_NWM_agg_sliced.iterrows():
        ...
        #lons, lats, NWM_ID = get_NWM_poly(row['fid_1'],NWM_polygon)
        lons, lats, NWM_ID = get_NWM_poly(row['NWMcommID'],NWM_polygon)

        # Reset for now the id naming convention for NWMv3.0
        NWM_ID = int(row['NWMcommID'])
...
def main(args):
    ...
    mask, feature_ids, fidx = extract_offsets(f0, NWM_streams['NWMcommID'].values)
    ...
    inflows = np.ma.masked_array(np.zeros((len(files), len(feature_ids))), fill_value=0.0)

    data = {'ids': ids_final,
            'col_index': ids_final,
            'row_index': list(files.keys())}

    print("Reading NWM polygon inflows...")
    ffiles = len(files)
    for i, f in enumerate(files.values()):
        inflows[i] = extract_inflow_NWM(f,mask)
