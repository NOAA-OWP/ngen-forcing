...
def mesh_create_polygon(coord_list, cat_id, cleanup_geometry=False):
    ...
    if cleanup_geometry:
        ...
        #the following cat-ids are specific for huc01 and vsersion of hydrofabric. For general case, users need to identify the cat-ids
        #that cause meshing to fail
...
def grid_to_mesh_regrid(cat_id, lons_start, lats_start, lons_min, lons_max, lats_min, lats_max, polygon, esmf_outfile, mesh,
    ...
    # Add corners to the source grid.
    #srcgrid.add_corners(staggerloc=[ESMF.StaggerLoc.CORNERS])
    ...
    #srcfield.read(filename=datafile_in, variable="TMP_2maboveground", timeslice=1)
    #srcfield_temp = srcfield.data[:,:]
...
def csv_to_netcdf(num_catchments, aorc_ncfile, regrid_weight_forcing):
    """
    ...
    if regrid_weight_forcing:
        ...
        output_path = join(output_root, forcing, "huc01_forcing.nc")
    else:
        csv_infile = join(input_root, mask_avg, "avg_output.csv")
        output_path = join(output_root, mask_avg, "huc01_forcing.nc")
...
if __name__ == '__main__':
    ...
    #hyfabfile = "/local/ngen/data/10259000/spatial/catchment_data.geojson"
    ...
    geomtype = cat_df_full.geometry[0].geom_type    #pick first row of Polygon or MultiPolygon, assume all are the same geom_type
    ...
    # mask based average
    avg_outfile = join(output_root, mask_avg, "avg_output.csv")
    ...
    #datafile_path = join("/local/ngen/data/10259000/forcing", aorc_netcdf, "AORC-OWP_*.nc4")
    ...
    regrid_weight_forcing = True    #calculate weight averaged forcing
    csv_to_netcdf(num_catchments, aorc_ncfile, regrid_weight_forcing)
    regrid_weight_forcing = False    #calculare maske averaged forcing
    csv_to_netcdf(num_catchments, aorc_ncfile, regrid_weight_forcing)
