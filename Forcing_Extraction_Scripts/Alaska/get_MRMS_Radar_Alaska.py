...
class MRMSRadarAlaskaDownloader(FixedFileDownloader, ABC):
    """Downloader for MRMS RadarOnly hourly QPE for Alaska.
    ...
    def get_file_specs(self, d_start):
        ...
        subdir = f"Alaska/RadarOnly_QPE_01H_00.00/{d_start.strftime('%Y%m%d')}"
        filename = f"MRMS_RadarOnly_QPE_01H_00.00_{d_start.strftime('%Y%m%d')}-{d_start.strftime('%H')}0000.grib2.gz"
        return [(subdir, filename)]


if __name__ == "__main__":
    downloader = MRMSRadarAlaskaDownloader.from_cli_args()
    downloader.run()
