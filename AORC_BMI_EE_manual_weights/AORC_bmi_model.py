...
class AORC_bmi_model(Bmi):
    ...
    #----------------------------------------------
    _att_map = {
        'model_name':         'AORC_Forcings_EE_Weights_Manual',
        'version':            '1.0',
    ...
    #-------------- Initalize AORC Forcings specifications for NextGen -----#
        ...
        # Get the user specified ExactExtract weights file for manual regridding to hydrofabric
        self._EE_weights = pd.read_csv(self.cfg_bmi['EE_weights'])

        # Now allocate a AORC forcings array storing data and
        # the EE summation total for all AORC met variables
        # within EE manual regrid loop
        self._AORC_data = np.zeros((len(self._AORC_met_vars),nc_file.variables['latitude'].shape[0],nc_file.variables['longitude'].shape[0]))
        self._EE_data = np.zeros((len(self._AORC_met_vars),len(self._EE_weights)))
    ...
    def update_until(self, future_time: float):
        ...
        self._model.run(self._values, update_delta_t, self._date, self._base_url, self._aorc_beg, self._aorc_end, self._aorc_new_end, self._AORC_met_vars, self._scale_factor, self._add_offset, self._hyfabfile, self._EE_weights, self._EE_data, self._AORC_data)
    #------------------------------------------------------------    
    def finalize( self ):
        """Finalize model."""
        self._model = None
