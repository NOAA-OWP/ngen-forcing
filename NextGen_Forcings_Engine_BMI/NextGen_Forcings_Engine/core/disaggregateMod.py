...
def disaggregate_factory(ConfigOptions):
    ...
    elif len(ConfigOptions.supp_precip_forcings) == 1 and ConfigOptions.supp_precip_forcings[0] == 12:
...
def ak_ext_ana_disaggregate(input_forcings, supplemental_precip, config_options, mpi_config):
    """
    ...
    if(config_options.grid_type=="unstructured"):
        ...
        if input_forcings.regridded_forcings2 is None or supplemental_precip.regridded_precip2 is None:
            ...
            return
        # Check to make sure we have valid grids.
        if input_forcings.regridded_forcings2_elem is None or supplemental_precip.regridded_precip2_elem is None:
            if mpi_config.rank == 0:
                config_options.statusMsg = "Bypassing ak_ext_ana_disaggregation routine due to missing input or supp pcp data"
                err_handler.log_warning(config_options, mpi_config)
            return
    ...
    if supplemental_precip.ext_ana != "STAGE4":
        ...
        if(config_options.grid_type=="gridded"):
            ...
            supplemental_precip.regridded_precip2[:] = input_forcings.regridded_forcings2[3,:]
            supplemental_precip.regridded_precip2_elem[:] = input_forcings.regridded_forcings2_elem[3,:]
        elif(config_options.grid_type=="hydrofabric"):
            supplemental_precip.regridded_precip2[:] = input_forcings.regridded_forcings2[3,:]
        if mpi_config.rank == 0:
    ...
    if(config_options.grid_type=="gridded"):
        ...
        disagg_factors = np.select([ana_all_zeros,(ana_no_zeros | target_data_no_zeros)],
                  [1/6.0*np.ones(supplemental_precip.regridded_precip2[:].shape),np.clip(target_data/ana_sum,0,1)],
                  0)
        disagg_factors_elem = np.select([ana_all_zeros,(ana_no_zeros | target_data_no_zeros)],
                  [1/6.0*np.ones(supplemental_precip.regridded_precip2_elem[:].shape),np.clip(target_data/ana_sum,0,1)],
                  0)
    elif(config_options.grid_type=="hydrofabric"):
        disagg_factors = np.select([ana_all_zeros,(ana_no_zeros | target_data_no_zeros)],
                  [1/6.0*np.ones(supplemental_precip.regridded_precip2[:].shape),np.clip(target_data/ana_sum,0,1)],
    ...
    if(config_options.grid_type=="gridded"):
        ...
        np.nan_to_num(supplemental_precip.regridded_precip2_elem[:], copy=False, nan=config_options.globalNdv)
    elif(config_options.grid_type=="hydrofabric"):
        supplemental_precip.regridded_precip2[:] *= disagg_factors
        np.nan_to_num(supplemental_precip.regridded_precip2[:], copy=False, nan=config_options.globalNdv)
