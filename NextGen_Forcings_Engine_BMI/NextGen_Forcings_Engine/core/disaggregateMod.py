...
def disaggregate_factory(ConfigOptions):
    """Disaggregation function factory."""
    if (
        len(ConfigOptions.supp_precip_forcings) == 1
        and ConfigOptions.supp_precip_forcings[0] == 11
    ):
        return ak_ext_ana_disaggregate
    elif (
        len(ConfigOptions.supp_precip_forcings) == 1
        and ConfigOptions.supp_precip_forcings[0] == 12
    ):
...
def no_op_disaggregate(input_forcings, supplemental_precip, config_options, mpi_config):
    """No operation disaggregation function."""
    pass


def ak_ext_ana_disaggregate(
    input_forcings, supplemental_precip, config_options, mpi_config
):
    """Disaggregation function for 6hr SuppPcp data to 1hr Input data.
    ...
    if config_options.grid_type == "gridded":
        ...
        if (
            input_forcings.regridded_forcings2 is None
            or supplemental_precip.regridded_precip2 is None
        ):
            if mpi_config.rank == 0:
                config_options.statusMsg = "Bypassing ak_ext_ana_disaggregation routine due to missing input or supp pcp data"
    ...
    if config_options.grid_type == "unstructured":
        ...
        # Check to make sure we have valid grids.
        if (
            input_forcings.regridded_forcings2_elem is None
            or supplemental_precip.regridded_precip2_elem is None
        ):
            if mpi_config.rank == 0:
                config_options.statusMsg = "Bypassing ak_ext_ana_disaggregation routine due to missing input or supp pcp data"
                err_handler.log_warning(config_options, mpi_config)
    ...
    if supplemental_precip.ext_ana != "STAGE4":
        ...
        if config_options.grid_type == "gridded":
            supplemental_precip.regridded_precip2[:, :] = (
                input_forcings.regridded_forcings2[3, :, :]
            )
            ...
            supplemental_precip.regridded_precip2_elem[:] = (
                input_forcings.regridded_forcings2_elem[3, :]
            )
        elif config_options.grid_type == "hydrofabric":
            supplemental_precip.regridded_precip2[:] = (
                input_forcings.regridded_forcings2[3, :]
    ...
    if mpi_config.rank == 0:
        ...
        beg_hh, end_hh, yyyymmdd = (
            date_iter.strftime("%H"),
            end_date.strftime("%H"),
            date_iter.strftime("%Y%m%d"),
        )
    ...
    if mpi_config.rank == 0:
        config_options.statusMsg = (
            f"Performing hourly disaggregation of {supplemental_precip.file_in2}"
        )
        err_handler.log_msg(config_options, mpi_config)
    ...
    ana_all_zeros = mpi_config.scatter_array(
        input_forcings, ana_all_zeros, config_options
    )
    err_handler.check_program_status(config_options, mpi_config)
    ana_no_zeros = mpi_config.scatter_array(
        input_forcings, ana_no_zeros, config_options
    )
    err_handler.check_program_status(config_options, mpi_config)
    target_data_no_zeros = mpi_config.scatter_array(
        input_forcings, target_data_no_zeros, config_options
    )
    err_handler.check_program_status(config_options, mpi_config)
    ...
    if config_options.grid_type == "gridded":
        disagg_factors = np.select(
            [ana_all_zeros, (ana_no_zeros | target_data_no_zeros)],
            [
                1 / 6.0 * np.ones(supplemental_precip.regridded_precip2[:, :].shape),
                np.clip(target_data / ana_sum, 0, 1),
            ],
            0,
        )
    elif config_options.grid_type == "unstructured":
        ...
        disagg_factors_elem = np.select(
            [ana_all_zeros, (ana_no_zeros | target_data_no_zeros)],
            [
                1 / 6.0 * np.ones(supplemental_precip.regridded_precip2_elem[:].shape),
                np.clip(target_data / ana_sum, 0, 1),
            ],
            0,
        )
    ...
    if config_options.grid_type == "gridded":
        ...
        supplemental_precip.regridded_precip2_elem[:] *= disagg_factors_elem
        np.nan_to_num(
            supplemental_precip.regridded_precip2_elem[:],
            copy=False,
            nan=config_options.globalNdv,
