...
class AORC_bmi_model(Bmi):
    ...
    def __init__(self):
        ...
        self._values = {}
        self._var_loc = "face"      # JG Edit
        self._var_grid_id = 6       # JG Edit
    ...
    #----------------------------------------------
    _att_map = {
        'model_name':         'AORC_Forcings_BMI_Python',
        'version':            '1.0',
    ...
    #-------------- Initalize AORC Forcings specifications for NextGen -----#
        ...
        self._base_url = 'https://path/to/errdap/server'
        ...
        for model_input in self.get_input_var_names():
            ...
            self._values[model_input] = np.zeros(self._grid_shape, dtype=float)
        for model_output in self.get_output_var_names():
            #self._values[model_output] = np.zeros(self.var_array_lengths, dtype=float)
            self._values[model_output] = np.zeros(self._grid_shape, dtype=float)
    ...
    def update_until(self, future_time: float):
        ...
        self._model.run(self._values, update_delta_t, self._date, self._base_url, self._aorc_beg, self._aorc_end, self._aorc_new_end, self._AORC_met_vars, self._scale_factor, self._add_offset, self._missing_value)
    #------------------------------------------------------------    
    def finalize( self ):
    ...
    def get_var_grid(self, name):
        ...
        # all vars have grid 5 but check if its in names list first
    ...
    #------------------------------------------------------------ 
    def get_grid_type(self, grid_id=6):
